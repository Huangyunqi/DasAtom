...
class Init(Inst):
    ...
    def operate(
        ...
        for i, row_id in enumerate(aod_col_act_idx): # ??? aod_row_act_idx ???
...
class Reload(ComboInst):
    ...
    def add_row_reload(self, r: int):
        ...
        #print("add_row_reload:::")
        #print(self.insts[-1].emit())
        return self.insts[-1]
...
class CodeGen():
    ...
    def builder(self, no_transfer: bool):
        ...
        #print("------------builder_init----------------")
        #for items in program.emit_full():
        #    print(items)
        # time_builder_swap = 0
        ...
        for s in range(1, len(self.layers)):
            ...
            if (not no_transfer) or s == 1:
                ...
                t_s = time.time()
                #print("--------------builder_reload begin---------------")
                self.builder_reload(s, cols, rows, qubits, program)
                #print("--------------builder_reload full---------------")
                #for items in program.emit_full():
                #    print(items)
            ...
            # t_s = time.time()
            # print("builder_move")
            #print("--------------------builder_move begin----------------")
            self.builder_move(s, cols, rows, qubits, program)
            #print("--------------------builder_move full----------------")
            #for items in program.emit_full():
            #    print(items)
    ...
    def builder_reload(
        ...
        print("builder_reload beginning::::::::::::::")
        layer = self.layers[s]
        prev_layer = self.layers[s-1]
        reload_obj = Reload(s)
        print("layer:")
        ...
        # reload row by row
        dict_site_qs = {}
        print(" for q_id in range(len(qubits)::")
        for q_id in range(len(qubits)):
            ...
            if (layer['qubits'][q_id]['a'] == 1):
                ...
                if row_id in dict_site_qs.keys():
                    ...
                    dict_site_qs[row_id] = {site_x: [q_id]}
        print("dict_site_qs:::::", dict_site_qs)
        print("enumerate row_id in dict_site_qs")
        for row_id in sorted(dict_site_qs):
            ...
            print("enumerate site_x in sorted(dict_site_qs[row_id]")
            for site_x in sorted(dict_site_qs[row_id]):
                print("site_x is:",site_x)
                site_qs = dict_site_qs[row_id][site_x]
                print("site_qs is::", site_qs)
                ...
                if len(site_qs) == 2:
                    ...
                    if layer['qubits'][q_id_left]['c'] >\
                        ...
                        q_id_right = tmp
                    #print("q_id_left:", q_id_left, "q_id_right",q_id_right)
                    ...
                    #print("current location of Cols:col_id_left", col_id_left, "col_id_right", col_id_right)
                    #print("lower_offset::", lower_offset, "upper_offset::", upper_offset)
                    # target locations of Cols
                    lower_x = qubits[q_id_left].x
                    upper_x = qubits[q_id_right].x
                    #print("target location of Cols: lower_x:", lower_x, "upper_x:", upper_x)
                    # process the Col on the left
                    #print("process Col on the left, col_id_left need to be active")
                    if not cols[col_id_left].active:
                        #print("If not active")
                        cols_to_active.append(col_id_left)
                        x_to_activate.append(lower_x)
                        #print("cols_to_active:", cols_to_active, "x_to_activate:", x_to_activate)
                    else:
                        #print("if active")
                        ...
                        #print(reloadRow_obj.emit())

                    # process the Col on the right
                    #print("process Col on the right, col_id_right need to be active")
                ...
                #print("Shift other Cols due to AOD order constraints")
                for col_id in layer['x_cols_begin'][site_x]:
                    ...
                    if (cols[col_id].active
                        ...
                        if layer['col'][col_id]['offset_begin'] >\
                                upper_offset:
                            #print("layer['col'][col_id]['offset_begin'] > upper_offset",layer['col'][col_id]['offset_begin'],">",upper_offset )
                            if layer['col'][col_id]['offset_begin'] - upper_offset == 1:
                                ...
                                #pirnt(reloadRow_obj.emit())
                            

                        # if there is a col on the left of the cols for loading
                        elif layer['col'][col_id]['offset_begin'] <\
                                lower_offset:
                            #print("layer['col'][col_id]['offset_begin'] < lower_offset",layer['col'][col_id]['offset_begin'],"<",lower_offset )
                ...
                pickup_qs += site_qs
                #print("pickup_qs add", pickup_qs)

            # print("pickup_qs")
            # print(pickup_qs)
            # input()
            # apply all the col shifts added previously
            #print("apply all col shift")
            ...
            #print("shift down the finished row")
    ...
    def builder_offload(
        ...
        #print("row-by-row processiong, self.r_high is:",self.r_high)
        for row_id in range(self.r_high):
            #print("row_id is::", row_id)
            if rows[row_id].active:
                #print("active")
                ...
                dict_site_q_slm = {}
                dict_site_q_aod = {}
                #print("begin enumerate layer['qubit']")
                for q_id, q in enumerate(layer['qubits']):
                    #print("q_id is::", q_id, "q is::", q)
                    if q['y'] == layer['row'][row_id]['y_end']:
                        #print("If q['y'] == layer['row'][row_id]['y_end']")
                        site_x = q['x'] 
                        #print("site_x = q['x'] is::", site_x)
                        ...
                        #print("dict_site_q_slm is:", dict_site_q_slm)
                        #print("dict_site_q_aod is:", dict_site_q_aod)
                #print("enumerate sorted(dict_site_q_slm)")
                for site_x in sorted(dict_site_q_slm):
                    #print("site_x is:",site_x)
                    site_q_slm = dict_site_q_slm[site_x]
                    site_q_aod = dict_site_q_aod[site_x]
                    #print("site_q_slm is:",site_q_slm)
                    #print("site_q_aod is:", site_q_aod)
                ...
                #                 site_q_slm.append(q_id)
                    if len(site_q_aod) == 2:
                        ...
                        #print("col_id_left is:", col_id_left, "col_id_right is:", col_id_right)
                        #print("lower_offset is:", lower_offset, "upper_offset is:", upper_offset)
                        #print("lower_x is:", lower_x, "upper_x is:",upper_x)
                        ...
                        #print(offloadRow_obj.emit())
                    elif len(site_q_aod) == 1:
                        #print("len(site_q_aod) == 1")
                        q_id = site_q_aod[0]
                        #print("q_id is:", q_id)
                        col_id_left = layer['qubits'][q_id]['c']
                        col_id_right = col_id_left
                    ...
                    #print("enumerate col_id in layer['x_cols_end'][site_x]")
                    for col_id in layer['x_cols_end'][site_x]:
                        ...
                        if (cols[col_id].active
                            ...
                            if layer['col'][col_id]['offset_end'] >\
                                    upper_offset:
                                #print("layer['col'][col_id]['offset_end'] >upper_offset")
                                offloadRow_obj.add_col_shift(
                                    id=col_id,
                                    begin=cols[col_id].x,
                                    end=upper_x + AOD_SEP *
                                ...
                                #print("id is:",col_id, "begin is:", cols[col_id].x, "end is:",upper_x + AOD_SEP *(layer['col'][col_id]['offset_end'] -upper_offset) + 1)
                            elif layer['col'][col_id]['offset_end'] <\
                                    lower_offset:
                                #print("layer['col'][col_id]['offset_end'] < lower_offset")
                                offloadRow_obj.add_col_shift(
                                    id=col_id,
                                    begin=cols[col_id].x,
                                    end=lower_x + AOD_SEP *
                                ...
                                #print("id is:", col_id, "begin is:",cols[col_id].x, "end is:", lower_x + AOD_SEP *(layer['col'][col_id]['offset_end'] -lower_offset) - 1)
                            else:
                                #print("else")
                                offloadRow_obj.add_col_shift(
                                    id=col_id,
                                ...
                                #print("id is:", col_id, "begin is:",cols[col_id].x, "end is:",lower_x + AOD_SEP *(layer['col'][col_id]['offset_end'] -lower_offset) )
                            #print(offloadRow_obj.emit())
